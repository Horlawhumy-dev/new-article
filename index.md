...
The next thing is to create a template folder inside the chat app folder. Then make a chat app folder, and inside that, make an `index.html` file.
...
Each of the channels has a name and any client having the name can send a message in as much server is still on.
Channel layers also can be grouped. Every group has a name and anyone can remove or add a channel having the precise name.
